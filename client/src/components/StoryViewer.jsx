...
const StoryViewer = ({viewStory, setViewStory}) => {
    ...
    useEffect(()=>{
        ...
        if(viewStory && viewStory.media_type !== 'video'){
            ...
            progressInterval = setInterval(() => {
                elpased += setTime;
                setProgress((elpased / duration) * 100);
            }, setTime);
            // tutup story
            timer = setTimeout(()=>{
                setViewStory(null)
  ...
  return (
    // !BUG - ada bug di bg color nya 2.22.00 UPDATE solved = background_color <> backgroundColor
    <div className='fixed inset-0 h-screen bg-black bg-opacity-90 z-110 flex items-center justify-center' style={{backgroundColor:  viewStory.media_type === 'text' ? viewStory.background_color : '#000000'}}> 
        ...
        {/* User info  */}
        {/* !BUG masih ada bug - UPDATE solved = <div className='text-white font-medium flex items-center gap-1.5'>*/}
        ...
        {/* Close button */}
        <button onClick={handleClose} className='absolute top-4 right-4 text-white text-3x; font-bold focus:outline-none'>
